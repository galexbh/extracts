...
export default function Productos() {
  ...
  useEffect(() => {
    Promise.all([cargarProductos(), cargarEstados()]).finally(() => setLoading(false));
  ...
  const { totalProductos, prodActivos, prodInactivos } = React.useMemo(() => {
    ...
    const activos = data.filter((r) => {
      const estado = (r.estado_producto || r.nombre_estado || "").toString().toLowerCase();
      return estado === "activo";
    }).length;
    return { totalProductos: total, prodActivos: activos, prodInactivos: total - activos };
  }, [data]);
  ...
  const getFilteredData = useCallback(
    (filters = {}) => {
      ...
      if (filters.estado) {
        filtered = filtered.filter((r) => (r.estado_producto || "").toLowerCase() === filters.estado.toLowerCase());
      }
      return filtered;
    },
    [data]
  ...
  const handleExportPDF = async (filters = {}) => {
    try {
      ...
      let y = finalY + 18;
      doc.text(`Total de productos exportados: ${rows.length}`, 50, y); y += 16;
      const act = rows.filter((r) => (r.estado_producto || "").toLowerCase() === "activo").length;
  ...
  const handleExportExcel = async (filters = {}) => {
    try {
      ...
      const allCols = [
        { key: "id", header: "ID", width: 8, extract: (r) => r.id_producto },
          ...
          <Flex justify="space-between" align="center" wrap="wrap" gap={3}>
            <Box>
              <HStack spacing={2}>
                ...
                <Heading size="md" color={accent}>Gestión de Productos</Heading>
              </HStack>
          ...
          <SimpleGrid columns={{ base: 1, md: 3 }} spacing={4} mt={5} mb={2}>
            <Stat bg={statTotalBg} p={4} borderRadius="lg">
              <HStack spacing={2} mb={1}>
                ...
                <StatLabel fontWeight="bold" color="teal.800">Total de Productos</StatLabel>
              </HStack>
              <StatNumber fontSize="3xl" color="teal.900">{totalProductos}</StatNumber>
            ...
            <Stat bg={statActivosBg} p={4} borderRadius="lg">
              <HStack spacing={2} mb={1}>
                ...
                <StatLabel fontWeight="bold" color="green.800">Prod. Activos</StatLabel>
              </HStack>
              <StatNumber fontSize="3xl" color={activosNumberColor}>{prodActivos}</StatNumber>
              <StatHelpText m={0} color="green.700">Listos para facturar</StatHelpText>
            </Stat>
        ...
        <ModalContent>
          ...
          <ModalBody py={5}>
            ...
            <FormControl mb={3}>
              ...
              <Input placeholder="Ej: Jugo de Naranja" value={expNombre} onChange={(e) => setExpNombre(e.target.value)} size="sm" bg={modalInputBg} />
            </FormControl>
